<html>
<head>
<title>js-binarypack Test</title>

<script src="../dist/binarypack.js"></script>

<script>

function measureBinaryPack(times) {
  var data = document.getElementById('json-data').value;
  var reference = JSON.stringify(data);
  var start = new Date();
  var packedVal;
  for (var i = 0; i < times; i++) {
    packedVal = BinaryPack.pack(data);
  }
  var packedEnd = new Date();
  blobToArrayBuffer(packedVal, function(ab) {
    var unpackedStart = new Date();
    for (var i = 0; i < times; i++) {
      unpackedVal = BinaryPack.unpack(ab);
    }
    var unpackedEnd = new Date();
    if (JSON.stringify(unpackedVal) == reference) {
      console.log('BinaryPack: Packed in', (packedEnd-start), 'ms. Unpacked in', (unpackedEnd-unpackedStart), 'ms');
    } else {
      console.log('Did not pack and unpack the same data');
    }
  });
}

function measureJSON(times) {
  var data = document.getElementById('json-data').value;
  var reference = JSON.stringify(data);
  var start = new Date();
  var packedVal;
  for (var i = 0; i < times; i++) {
    packedVal = JSON.stringify(data);
  }
  var packedEnd = new Date();

  var unpackedVal;
  for (var i = 0; i < times; i++) {
    unpackedVal = JSON.parse(packedVal);
  }
  var unpackedEnd = new Date();
  if (JSON.stringify(unpackedVal) == reference) {
      console.log('JSON: Packed in', (packedEnd-start), 'ms. Unpacked in', (unpackedEnd-packedEnd), 'ms');
  } else {
    console.log('Did not pack and unpack the same data');
  }
}
